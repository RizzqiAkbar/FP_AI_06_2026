...
const GOALS = [
  { value: 'weight loss',    label: 'Turunkan berat',  desc: 'Kurangi lemak tubuh' },
  { value: 'muscle gain',    label: 'Tambah otot',     desc: 'Tingkatkan massa otot' },
  { value: 'maintain',       label: 'Pertahankan',     desc: 'Jaga berat badan' },
  { value: 'healthy eating', label: 'Hidup sehat',     desc: 'Pola makan seimbang' },
];

const CONDITIONS = [
  { value: 'diabetes',     label: 'Diabetes' },
  { value: 'hypertension', label: 'Hipertensi' },
  { value: 'cholesterol',  label: 'Kolesterol' },
  { value: 'gout',         label: 'Asam Urat' },
  { value: 'normal',       label: 'Tidak ada' },
];
...
export default function HealthForm({ onSave }: HealthFormProps = {}) {
  ...
  const toggleCondition = (value: string) => {
    setProfile(p => {
      if (value === 'normal') {
        return { ...p, conditions: p.conditions.includes('normal') ? [] : ['normal'] };
      }
      const without = p.conditions.filter(c => c !== 'normal');
      return {
        ...p,
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const profileToSave = {
      ...profile,
      health_condition: profile.conditions.includes('normal')
        ? 'normal'
  ...
  const formContent = (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="bg-white rounded-[14px] p-6" style={{ border: '0.5px solid #d4e8c2' }}>
        ...
        {!profile.conditions.includes('normal') && (
          <div className="mt-4">
            <label className="block text-sm font-medium mb-1.5" style={{ color: '#444441' }}>
              Deskripsi kondisi kesehatan
            </label>
            <input
              ...
              style={inputStyle}
            />
            <p className="text-xs mt-1.5" style={{ color: '#5F5E5A' }}>
              Dideskripsikan ke AI untuk analisis yang lebih personal
            </p>
          </div>
        )}
      </div>
      ...
      >
        {isModal ? 'Simpan perubahan' : 'Simpan & lanjutkan →'}
      </button>
